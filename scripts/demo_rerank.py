...
Steps:
    1. Ingest a document into chunks with the existing pipeline.
    2. Generate embeddings via LM Studio (OpenAI-compatible /v1/embeddings).
    3. Persist chunks + embeddings in PostgreSQL with pgvector.
    4. Run a dense ANN search (top candidate_k) and re-rank with a Cross-Encoder
...
def embed_with_lm_studio(
    ...
    *,
    ...
    """Call LM Studio's /v1/embeddings endpoint and return one vector per text."""
    out: list[list[float]] = []
    for start in range(0, len(texts), batch_size):
        ...
        for _ in range(3):
            try:
                ...
                last_error = exc
                time.sleep(1)
        else:  # pragma: no cover - exercised only on network failure
            raise RuntimeError("LM Studio embeddings endpoint failed") from last_error
    return out
...
def main() -> None:
    ...
    parser.add_argument("--reset-table", action="store_true", help="Drop the embeddings table before inserting.")
    parser.add_argument("--reranker-model", default="cross-encoder/ms-marco-MiniLM-L-6-v2")
    parser.add_argument("--storage-model", default="bge-m3", help="Storage discriminator (must be in EMBEDDING_MODEL_DIMENSIONS).")
    args = parser.parse_args()

    base_url = os.environ.get("LM_STUDIO_BASE_URL", "http://localhost:1234")
    lm_studio_model = os.environ.get("LM_STUDIO_EMBEDDING_MODEL", "text-embedding-bge-m3")
    ...
    sample = chunks[0]
    print(f"sample chunk[0]   = page={sample.metadata.page} chars={len(sample.text)} hash={sample.content_hash[:12]}...")

    _print_section("2) EMBEDDING (LM Studio)")
    print(f"lm_studio_url     = {base_url}")
    print(f"lm_studio_model   = {lm_studio_model}")
    t0 = time.perf_counter()
    vectors = embed_with_lm_studio(
    ...
    query_vector = embed_with_lm_studio(
        [args.query], base_url=base_url, model=lm_studio_model
    )[0]
    ...
    for rank, candidate in enumerate(dense_candidates, start=1):
        ...
        print(f"{rank:>4} {candidate.cosine_similarity:>8.4f} {page!s:>5} {idx!s:>4}  {preview}")
